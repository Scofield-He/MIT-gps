...
def plot_tec_prof(year, glon, lt):
    ...
    for date, doy in df.index:
        ...
        plt.scatter(lats, mean_tec, c='b', marker='o', s=50, label="TEC")
        ...
        if cur_min:                                             # 非0的数据为判定槽极小的位置
            plt.scatter(cur_min, mean_tec.loc["gdlat-{}°".format(cur_min)], c='r', marker='^', s=80, label='trough min')

        figure_title = '{}  doy:{:3d}  lt:{}h  glon:{}  kp:{}'.format(date, doy, lt, glon, kp)
        # figure_title = '{} lt:{}-{}  glon:{}'.format(date, lt, lt + 1, glon)
        plt.legend()
        plt.title(figure_title)
        fig.savefig(out_path + "{} {:03d} lt_{} glon_{}".format(year, doy, lt, glon))
        plt.close()
